...
#include "../../easy_physics/collision/EpxVectorFunction.h"
...
static void render(void)
{
	...
	for(int i=0;i<physicsGetNumRigidbodies();i++) {
		...
		if(collidable.m_numCoreSpheres == 0) {
			...
			EpxVector3 extent = absPerElem(orientation) * ( collidable.m_half);
			...
			EpxVector3 radius(length(collidable.m_half));
		...
		renderDebugAabb(0.5f*(aabbMax+aabbMin),0.5f*(aabbMax-aabbMin),EpxVector3(1,0,0));
	}
	...
	for(int i=0;i<physicsGetNumRigidbodies();i++) {
		...
		for(int j=0;j<collidable.m_numCoreSpheres;j++) {
			EpxTransform3 worldTransform = rigidBodyTransform * EpxTransform3::translation(collidable.m_coreSpheres[j].m_offsetPosition);
